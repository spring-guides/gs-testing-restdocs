...
(from Spring Boot), which takes an argument for a directory location for the generated
...
There are also command line examples for `curl` and `httpie` (two common and popular command
line HTTP clients).
